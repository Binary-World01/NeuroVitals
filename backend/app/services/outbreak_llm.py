...
def _analyze_with_mock(prompt: str, data: dict) -> str:
    ...
    if "pain" in str(data).lower(): condition = "Muscle Strain / Fatigue"
    if "cough" in str(data).lower(): condition = "Upper Respiratory Infection"
    
    return f"""POSSIBLE CONDITION: {condition} (Demo Mode)
EXPLANATION: Based on the reported symptoms, this appears to be a common clinical presentation.
NEXT STEPS: 1. Rest and hydration. 2. Monitor temperature. 3. Consult a physician if symptoms persist.
DISCLAIMER: This is a DEMO MODE analysis (AI providers currently unreachable). Not a medical diagnosis."""
